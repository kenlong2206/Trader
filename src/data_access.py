...
def get_data_file() -> str:
    ...
    if is_test == 'true':
        data_file = os.path.join(PROJECT_ROOT, 'Trader', 'data', 'trader_data_test.txt')
    else:
        data_file = os.path.join(PROJECT_ROOT, 'Trader', 'data', 'trader_data.txt')
...
def create_trade(trade: Trade) -> Trade:
    ...
    logger.info(f"create_trade", trade)
    trade.trade_id = str(uuid.uuid4())
    trade.trade_status = "new"
    trade.created_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
